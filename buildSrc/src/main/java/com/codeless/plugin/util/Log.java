...
public class Log {
    ...
    static boolean learnMode = false;
    ...
    public static void setLearn(boolean learn) {
        //此处如果不加@会导致循环调用
        Log.learnMode = learn;
    }

    public static void learn(String msg) {
//        if (!enable || logImp == null) return
//        if (learnMode) {
        logImp.i("learn", msg);
//        }
    }
...
//    static void logEach(Object... msg) {
////        if (!enable || logImp == null) return
//
...
//            }
//        }
//        logImp.i("each", builder.toString());
//    }
...
//        println "logImp == null ? ${logImp == null}"
...
//        if (!enable || logImp == null) return
    ...
    public static String accCode2String(int access) {
        ...
        Map<Integer, String> map = getCheckedAccCodeMap();
        for (Map.Entry<Integer, String> next : map.entrySet()) {
            if ((next.getKey() & access) > 0) {
                builder.append('\t').append(next.getValue()).append('\t');
            }
        }
